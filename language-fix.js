...
function closeLanguageMenu(sw){
  if(!sw)return;
  sw.classList.remove('open');
  const menu=sw.querySelector('.lang-menu');
  if(menu){menu.style.setProperty('display','none','important');menu.hidden=true;}
  const button=sw.querySelector('#langToggle');
  if(button){button.setAttribute('aria-expanded','false');button.blur();}
}
function openLanguageMenu(sw){
  if(!sw)return;
  const menu=sw.querySelector('.lang-menu');
  if(menu){menu.hidden=false;menu.style.removeProperty('display');}
  sw.classList.add('open');
  sw.querySelector('#langToggle')?.setAttribute('aria-expanded','true');
}
function apply(en){
  en=!!en;
  const sw=document.getElementById('langSwitch');
  closeLanguageMenu(sw);
  translate(en);
  closeLanguageMenu(sw);
  requestAnimationFrame(()=>{translate(en);closeLanguageMenu(sw);community(en);fixFAQ(en)});
  [0,50,150,300].forEach(ms=>setTimeout(()=>closeLanguageMenu(sw),ms));
}
...
function addNat5Nav(){
  ...
  if(!nav || nav.querySelector('a[href="nat5.html"]'))return;
  const a=document.createElement('a');
  a.href='nat5.html';
  a.textContent='⭐ NAT 5';
  a.setAttribute('aria-label','Nat 5 — Tier List e Team Builder');
  nav.appendChild(a);
}
function setup(){const sw=document.getElementById('langSwitch');if(!sw)return;rewardLayoutFix();activeCodesGuard();addNat5Nav();document.addEventListener('click',e=>{const t=e.target?.closest?.('#langENOption,#langPTOption,#langToggle');if(!t)return;e.preventDefault();e.stopImmediatePropagation();if(t.id==='langENOption'){closeLanguageMenu(sw);apply(true)}else if(t.id==='langPTOption'){closeLanguageMenu(sw);apply(false)}else{if(sw.classList.contains('open'))closeLanguageMenu(sw);else openLanguageMenu(sw)}},true);document.addEventListener('click',e=>{if(!sw.contains(e.target))closeLanguageMenu(sw)},false);let saved='pt';try{saved=localStorage.getItem('yunamyst-language')||localStorage.getItem('yunamyst-lang')||'pt'}catch(e){}apply(saved==='en');const root=document.getElementById('ativos')||document.body;let timer=0;new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(()=>{const en=document.documentElement.lang==='en';translate(en);community(en);fixFAQ(en);addNat5Nav();if(document.activeElement?.id==='langENOption'||document.activeElement?.id==='langPTOption')closeLanguageMenu(sw)},30)}).observe(root,{childList:true,subtree:true})}
