...
def fetch(url):
 r=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 (compatible; YunaMyst-Code-Updater/20.0)'})
...
def parse_rewards(raw, code):
 ...
 if i<0:return []
 chunk=text[max(0,i-500):i+1000]
 out=[]
 def add(kind,n):
  if n is not None and not any(k==kind for k,_ in out):out.append([kind,'x'+str(n)])
 add('energy',qty(chunk,['ENERGY','ENERGIA']))
 add('mana',qty(chunk,['MANA']))
 add('crystal',qty(chunk,['CRYSTAL','CRYSTALS','CRYSTAL(S)','CRISTAIS']))
 add('gold',qty(chunk,['MYSTICAL SCROLL','MYSTICAL','SCROLL MYSTICAL','SCROLLS MYSTICAL','SCROLL']))
 add('red',qty(chunk,['FIRE SCROLL','SCROLL FIRE','FIRE']))
 add('blue',qty(chunk,['WATER SCROLL','SCROLL WATER','WATER']))
 add('yellow',qty(chunk,['WIND SCROLL','SCROLL WIND','WIND']))
 return out

def merge_rewards(reward_hits):
 merged=[]
 for code,items in reward_hits.items():
  counts={}
  for source,data in items:
   for kind,amount in data:
    counts.setdefault((kind,amount),0);counts[(kind,amount)]+=1
  if counts:
   best=sorted(counts.items(),key=lambda x:(-x[1], ['energy','mana','crystal','gold','red','blue','yellow'].index(x[0][0]) if x[0][0] in ['energy','mana','crystal','gold','red','blue','yellow'] else 99))
   merged[code]=[list(k) for k,v in best if v>=1]
  else:merged[code]=[]
 return merged
...
def main():
 ...
 # Garantias para códigos conhecidos: mantemos os dados exatos quando as fontes os publicam de forma incompleta.
 known={'2SOREIKENIPPON6':[['mana','x200000'],['gold','x1']],'APAC1K0UB4NGK0K':[['energy','x100'],['gold','x1']],'AUGSW2026V7N':[['red','x3'],['energy','x100']],'SWXFRIEREN2026':[['energy','x100'],['mana','x300000'],['gold','x3']],'4READY4TDOT':[['mana','x200000'],['gold','x1']]}
 for c in active:
 ...
 payload={'updated':now,'source_count':len(SOURCES),'successful_sources':len(SOURCES)-len(errors),'trusted_confirmation':'1 trusted OR 2 independent','codes':active,'rewards':{c:rewards.get(c,[]) for c in active},'sources':details}
 CODES.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 HISTORY.write_text(json.dumps({'active':active,'expired':expired,'missing':{},'updated_at':now,'rewards':{c:rewards.get(c,[]) for c in active},'sources':details,'source_errors':errors},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print('Ativos:',', '.join(active));print('Recompensas:',json.dumps({c:rewards.get(c,[]) for c in active},ensure_ascii=False));print('Expirados removidos:',', '.join(sorted(REJECTED|confirmed_expired)))
