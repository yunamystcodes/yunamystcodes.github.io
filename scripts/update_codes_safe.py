...
def fetch(url):
 r=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 (compatible; YunaMyst-Code-Updater/18.0)'})
...
def main():
 ...
 try:old=json.loads(CODES.read_text(encoding='utf-8')); old_active={str(x).upper() for x in old.get('codes',[])}
 except Exception:old_active=set()
 # Só removemos automaticamente quando o código aparece explicitamente na secção de expirados de pelo menos uma fonte confiável, ou está na lista rejeitada.
 confirmed_expired={c for c,s in explicit.items() if len(s&TRUSTED)>=1 or len(s)>=2}
 active=sorted((old_active|confirmed)-REJECTED-confirmed_expired)
 ...
 old_exp=set(old.get('expired',[])) if isinstance(old,dict) else set()
